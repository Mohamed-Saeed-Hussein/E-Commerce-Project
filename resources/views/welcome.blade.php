...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col">
    @php
        $isLoggedIn = session('auth.user_id');
        $role = session('auth.role');
        $name = session('auth.name');
        $isAdmin = $isLoggedIn && $role === 'admin';
        $ctaUrl = $isLoggedIn ? ($isAdmin ? url('/admin') : url('/home')) : url('/login');
        $headline = $isAdmin
            ? 'Welcome back, Admin ' . ($name ?? '')
            : ($isLoggedIn ? 'Welcome back, ' . ($name ?? 'User') : 'Welcome to Style Haven');
        $subtext = $isAdmin
            ? 'Manage your store from the admin dashboard.'
            : ($isLoggedIn ? 'Explore our latest collection and enjoy shopping.' : 'Sign in to get started.');
    @endphp

    <!-- Auth-like welcome overlay (same background feel as login), two-column frame, slides down -->
    <section class="fixed inset-0 z-50 bg-gray-50 dark:bg-gray-900 flex items-center justify-center px-6 py-8">
        <div class="w-full max-w-5xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-0 bg-white dark:bg-gray-800 dark:border dark:border-gray-700 rounded-lg shadow overflow-hidden animate-welcome-slide md:min-h-[520px]">
                <!-- Text (left) -->
                <div class="order-2 md:order-1 flex items-center justify-center p-8 md:p-12 h-full animate-col-appear">
                    <div class="text-center md:text-left">
                        <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-4">{{ $headline }}</h1>
                        <p class="text-gray-600 dark:text-gray-300 mb-6">{{ $subtext }}</p>
                        <a href="{{ $ctaUrl }}" class="inline-flex items-center px-6 py-3 rounded-lg bg-primary-600 hover:bg-primary-700 text-white font-semibold">Get Started</a>
                    </div>
                </div>
                <!-- Image (right) - natural size, centered -->
                <div class="order-1 md:order-2 flex items-center justify-center p-6 md:p-8 h-full animate-col-appear">
                    <img src="{{ url('/images/photo-2.jpg') }}" alt="Welcome" class="max-w-full max-h-full h-auto object-contain">
                </div>
            </div>
        </div>
    </section>
    ...
    <style>
        @keyframes welcomeSlideDown { from { transform: translateY(-24px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        .animate-welcome-slide { animation: welcomeSlideDown .6s ease-out both; }
        @keyframes colAppear { from { opacity: 0; transform: scale(.98); } to { opacity: 1; transform: scale(1); } }
        .animate-col-appear { animation: colAppear .6s ease-out both; }
    </style>
    @php
        $isLoggedIn = session('auth.user_id');
        $role = session('auth.role');
        ...
        $subtext = $isAdmin
            ? 'Manage your store from the admin dashboard.'
            : ($isLoggedIn ? 'Explore our latest collection and enjoy shopping.' : 'Please sign in to get started.');
    @endphp
    ...
    <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
        <main class="flex max-w-[335px] w-full flex-col lg:max-w-4xl">
            <div class="text-[13px] leading-[20px] p-6 pb-12 lg:p-20 bg-white dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg text-center">
                <h1 class="mb-2 font-semibold text-2xl lg:text-3xl">{{ $headline }}</h1>
                <p class="mb-6 text-[#706f6c] dark:text-[#A1A09A]">{{ $subtext }}</p>
                <a href="{{ $ctaUrl }}" class="inline-block dark:bg-[#eeeeec] dark:border-[#eeeeec] dark:text-[#1C1C1A] dark:hover:bg-white dark:hover:border-white hover:bg-black hover:border-black px-5 py-2 bg-[#1b1b18] rounded-sm border border-black text-white text-sm leading-normal">
                    Get Started
                </a>
            </div>
        </main>
    ...
    {{-- Hide the right graphic column to keep only the welcome message --}}
    <style>.bg-\[\#fff2f2\], .dark\:bg-\[\#1D0002\]{display:none}</style>

    {{-- Preserve original inline assets (kept but visually hidden below) --}}
    <svg class="hidden" viewBox="0 0 440 376"></svg>
                <svg class="w-[448px] max-w-none relative -mt-[4.9rem] -ml-8 lg:ml-0 lg:-mt-[6.6rem] dark:hidden hidden" viewBox="0 0 440 376" fill="none" xmlns="http://www.w3.org/2000/svg">
